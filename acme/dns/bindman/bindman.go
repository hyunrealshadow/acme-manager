...
// Code from https://github.com/go-acme/lego/tree/v4.19.2/providers/dns/bindman
// License: MIT
...
const (
	...
	EnvManagerAddress     = "managerAddress"
	EnvPollingInterval    = "pollingInterval"
	EnvPropagationTimeout = "propagationTimeout"
)

const JsonSchema = "{\"$schema\":\"http://json-schema.org/draft-07/schema#\",\"title\":\"Bindman Configuration\",\"type\":\"object\",\"properties\":{\"credentials\":{\"managerAddress\":{\"type\":\"string\",\"description\":\"The server URL, should have scheme, hostname, and port (if required) of the Bindman-DNS Manager server\"}},\"additional\":{\"pollingInterval\":{\"type\":\"string\",\"description\":\"Time between DNS propagation check\"},\"propagationTimeout\":{\"type\":\"string\",\"description\":\"Maximum waiting time for DNS propagation\"},\"httpTimeout\":{\"type\":\"string\",\"description\":\"API request timeout\"}}}}"

var compiledJsonSchema *jsonschema.Schema

var credentialsFields = mapset.NewSet("managerAddress")
...
func init() {
	...
	if err != nil {
		logger.Fatalf("Failed to unmarshal bindman JSON schema: %v", err)
	}
	compiler := jsonschema.NewCompiler()
	if err := compiler.AddResource("schema.json", schema); err != nil {
		logger.Fatalf("Failed to add bindman schema resource to compiler: %v", err)
	}
	compiledJsonSchema, err = compiler.Compile("schema.json")
	if err != nil {
		logger.Fatalf("Failed to compile bindman schema: %v", err)
	}
}
...
func ParseConfig(conf *lego.DnsProviderConfig) (*lego.Env, error) {
	...
	if err := compiledJsonSchema.Validate(anyData); err != nil {
		return nil, fmt.Errorf("bindman: configuration validation failed: %v", err)
	}
...
func NewConfig(env lego.Env) *bindman.Config {
	return &bindman.Config{
...
func NewDNSProvider(env lego.Env) (*bindman.DNSProvider, error) {
