...
// Code from https://github.com/go-acme/lego/tree/v4.19.2/providers/dns/dreamhost
// License: MIT
...
func init() {
	...
	if err != nil {
		logger.Fatalf("Failed to unmarshal dreamhost JSON schema: %v", err)
	}
	compiler := jsonschema.NewCompiler()
	if err := compiler.AddResource("schema.json", schema); err != nil {
		logger.Fatalf("Failed to add dreamhost schema resource to compiler: %v", err)
	}
	compiledJsonSchema, err = compiler.Compile("schema.json")
	if err != nil {
		logger.Fatalf("Failed to compile dreamhost schema: %v", err)
	}
}
...
func ParseConfig(conf *lego.DnsProviderConfig) (*lego.Env, error) {
	...
	if err := compiledJsonSchema.Validate(anyData); err != nil {
		return nil, fmt.Errorf("dreamhost: configuration validation failed: %v", err)
	}
...
func NewConfig(env lego.Env) *dreamhost.Config {
	return &dreamhost.Config{
...
func NewDNSProvider(env lego.Env) (*dreamhost.DNSProvider, error) {
