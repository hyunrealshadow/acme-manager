...
// Code from https://github.com/go-acme/lego/tree/v4.19.2/providers/dns/gcore
// License: MIT
...
const (
	...
	EnvPermanentAPIToken      = "permanentApiToken"
	EnvPollingInterval        = "pollingInterval"
	EnvPropagationTimeout     = "propagationTimeout"
	EnvTTL                    = "ttl"
	defaultPollingInterval    = 20000000000
	defaultPropagationTimeout = 360000000000
)

const JsonSchema = "{\"$schema\":\"http://json-schema.org/draft-07/schema#\",\"title\":\"G-Core Configuration\",\"type\":\"object\",\"properties\":{\"credentials\":{\"permanentApiToken\":{\"type\":\"string\",\"description\":\"Permanent API token (https://gcore.com/blog/permanent-api-token-explained/)\"}},\"additional\":{\"pollingInterval\":{\"type\":\"string\",\"description\":\"Time between DNS propagation check\"},\"propagationTimeout\":{\"type\":\"string\",\"description\":\"Maximum waiting time for DNS propagation\"},\"ttl\":{\"type\":\"string\",\"description\":\"The TTL of the TXT record used for the DNS challenge\"},\"httpTimeout\":{\"type\":\"string\",\"description\":\"API request timeout\"}}}}"

var compiledJsonSchema *jsonschema.Schema

var credentialsFields = mapset.NewSet("permanentApiToken")
...
func init() {
	...
	if err != nil {
		logger.Fatalf("Failed to unmarshal gcore JSON schema: %v", err)
	}
	compiler := jsonschema.NewCompiler()
	if err := compiler.AddResource("schema.json", schema); err != nil {
		logger.Fatalf("Failed to add gcore schema resource to compiler: %v", err)
	}
	compiledJsonSchema, err = compiler.Compile("schema.json")
	if err != nil {
		logger.Fatalf("Failed to compile gcore schema: %v", err)
	}
}
...
func ParseConfig(conf *lego.DnsProviderConfig) (*lego.Env, error) {
	...
	if err := compiledJsonSchema.Validate(anyData); err != nil {
		return nil, fmt.Errorf("gcore: configuration validation failed: %v", err)
	}
...
func NewConfig(env lego.Env) *gcore.Config {
	return &gcore.Config{
...
func NewDNSProvider(env lego.Env) (*gcore.DNSProvider, error) {
