...
// Code from https://github.com/go-acme/lego/tree/v4.19.2/providers/dns/httpnet
// License: MIT
...
func init() {
	...
	if err != nil {
		logger.Fatalf("Failed to unmarshal httpnet JSON schema: %v", err)
	}
	compiler := jsonschema.NewCompiler()
	if err := compiler.AddResource("schema.json", schema); err != nil {
		logger.Fatalf("Failed to add httpnet schema resource to compiler: %v", err)
	}
	compiledJsonSchema, err = compiler.Compile("schema.json")
	if err != nil {
		logger.Fatalf("Failed to compile httpnet schema: %v", err)
	}
}
...
func ParseConfig(conf *lego.DnsProviderConfig) (*lego.Env, error) {
	...
	if err := compiledJsonSchema.Validate(anyData); err != nil {
		return nil, fmt.Errorf("httpnet: configuration validation failed: %v", err)
	}
...
func NewConfig(env lego.Env) *httpnet.Config {
	return &httpnet.Config{
...
func NewDNSProvider(env lego.Env) (*httpnet.DNSProvider, error) {
	...
	return httpnet.NewDNSProviderConfig(config)
}
