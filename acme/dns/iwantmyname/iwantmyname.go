...
// Code from https://github.com/go-acme/lego/tree/v4.19.2/providers/dns/iwantmyname
// License: MIT
...
func init() {
	...
	if err != nil {
		logger.Fatalf("Failed to unmarshal iwantmyname JSON schema: %v", err)
	}
	compiler := jsonschema.NewCompiler()
	if err := compiler.AddResource("schema.json", schema); err != nil {
		logger.Fatalf("Failed to add iwantmyname schema resource to compiler: %v", err)
	}
	compiledJsonSchema, err = compiler.Compile("schema.json")
	if err != nil {
		logger.Fatalf("Failed to compile iwantmyname schema: %v", err)
	}
}
...
func ParseConfig(conf *lego.DnsProviderConfig) (*lego.Env, error) {
	...
	if err := compiledJsonSchema.Validate(anyData); err != nil {
		return nil, fmt.Errorf("iwantmyname: configuration validation failed: %v", err)
	}
...
func NewConfig(env lego.Env) *iwantmyname.Config {
	return &iwantmyname.Config{
...
func NewDNSProvider(env lego.Env) (*iwantmyname.DNSProvider, error) {
	...
	return iwantmyname.NewDNSProviderConfig(config)
}
