...
// Code from https://github.com/go-acme/lego/tree/v4.19.2/providers/dns/netlify
// License: MIT
...
const JsonSchema = "{\"$schema\":\"http://json-schema.org/draft-07/schema#\",\"title\":\"Netlify Configuration\",\"type\":\"object\",\"properties\":{\"credentials\":{\"token\":{\"type\":\"string\",\"description\":\"Token\"}},\"additional\":{\"pollingInterval\":{\"type\":\"string\",\"description\":\"Time between DNS propagation check\"},\"propagationTimeout\":{\"type\":\"string\",\"description\":\"Maximum waiting time for DNS propagation\"},\"ttl\":{\"type\":\"string\",\"description\":\"The TTL of the TXT record used for the DNS challenge\"},\"httpTimeout\":{\"type\":\"string\",\"description\":\"API request timeout\"}}}}"
...
func init() {
	...
	if err != nil {
		logger.Fatalf("Failed to unmarshal netlify JSON schema: %v", err)
	}
	compiler := jsonschema.NewCompiler()
	if err := compiler.AddResource("schema.json", schema); err != nil {
		logger.Fatalf("Failed to add netlify schema resource to compiler: %v", err)
	}
	compiledJsonSchema, err = compiler.Compile("schema.json")
	if err != nil {
		logger.Fatalf("Failed to compile netlify schema: %v", err)
	}
}
...
func ParseConfig(conf *lego.DnsProviderConfig) (*lego.Env, error) {
	...
	if err := compiledJsonSchema.Validate(anyData); err != nil {
		return nil, fmt.Errorf("netlify: configuration validation failed: %v", err)
	}
...
func NewConfig(env lego.Env) *netlify.Config {
...
func NewDNSProvider(env lego.Env) (*netlify.DNSProvider, error) {
	...
	return netlify.NewDNSProviderConfig(config)
}
