...
// Code from https://github.com/go-acme/lego/tree/v4.19.2/providers/dns/servercow
// License: MIT
...
func init() {
	...
	if err != nil {
		logger.Fatalf("Failed to unmarshal servercow JSON schema: %v", err)
	}
	compiler := jsonschema.NewCompiler()
	if err := compiler.AddResource("schema.json", schema); err != nil {
		logger.Fatalf("Failed to add servercow schema resource to compiler: %v", err)
	}
	compiledJsonSchema, err = compiler.Compile("schema.json")
	if err != nil {
		logger.Fatalf("Failed to compile servercow schema: %v", err)
	}
}
...
func ParseConfig(conf *lego.DnsProviderConfig) (*lego.Env, error) {
	...
	if err := compiledJsonSchema.Validate(anyData); err != nil {
		return nil, fmt.Errorf("servercow: configuration validation failed: %v", err)
	}
...
func NewConfig(env lego.Env) *servercow.Config {
	return &servercow.Config{
...
func NewDNSProvider(env lego.Env) (*servercow.DNSProvider, error) {
	...
	return servercow.NewDNSProviderConfig(config)
}
