...
// Code from https://github.com/go-acme/lego/tree/v4.19.2/providers/dns/vultr
// License: MIT
...
func init() {
	...
	if err != nil {
		logger.Fatalf("Failed to unmarshal vultr JSON schema: %v", err)
	}
	compiler := jsonschema.NewCompiler()
	if err := compiler.AddResource("schema.json", schema); err != nil {
		logger.Fatalf("Failed to add vultr schema resource to compiler: %v", err)
	}
	compiledJsonSchema, err = compiler.Compile("schema.json")
	if err != nil {
		logger.Fatalf("Failed to compile vultr schema: %v", err)
	}
}
...
func ParseConfig(conf *lego.DnsProviderConfig) (*lego.Env, error) {
	...
	if err := compiledJsonSchema.Validate(anyData); err != nil {
		return nil, fmt.Errorf("vultr: configuration validation failed: %v", err)
	}
...
func NewConfig(env lego.Env) *vultr.Config {
	return &vultr.Config{
...
func NewDNSProvider(env lego.Env) (*vultr.DNSProvider, error) {
