...
// The AcmeServerFunc type is an adapter to allow the use of ordinary
// function as AcmeServer mutator.
type AcmeServerFunc func(context.Context, *ent.AcmeServerMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f AcmeServerFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.AcmeServerMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.AcmeServerMutation", m)
}
